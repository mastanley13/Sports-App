...
async function fetchContacts(query) {
    ...
    try {
        ...
        const contactsWithCustomFields = response.data.contacts.map(contact => {
            ...
            // Optionally, remove the customFields array since it's now unpacked
            // delete contact.customFields;
            return contact;
        });

        // Extract currentWeek from contacts or set a default value
        const currentWeek = contactsWithCustomFields[0]?.['Current Week'] || 1;

        return { contacts: contactsWithCustomFields, currentWeek };
    } catch (error) {
