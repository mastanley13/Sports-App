...
let threadId = process.env.VOICE_THREAD_ID;
...
async function createNewThread() {
    ...
    threadId = thread.id;
    console.log('New voice thread created:', threadId);
    updateEnvFile('VOICE_THREAD_ID', threadId);
    process.env.VOICE_THREAD_ID = threadId;
}

async function createNewVoiceCommandThread() {
    const thread = await openai.beta.threads.create();
    threadId = thread.id;
    console.log('New voice command thread created:', threadId);
    updateEnvFile('VOICE_THREAD_ID', threadId);
    process.env.VOICE_THREAD_ID = threadId;
}
...
async function handleVoiceCommand(message) {
    try {
        ...
        const audioFilePath = await generateSpeech(botResponse);

        return { content: botResponse, audioFilePath, functionCalls };
    } catch (error) {
...
module.exports = { handleVoiceCommand, createNewVoiceCommandThread };